...
use wgsparkl::rapier::math::{Isometry, Real, Vector};
...
impl EntityWithGraphics {
    ...
    pub fn spawn(
        ...
        materials: &mut Assets<BevyMaterial>,
        prefab_meshes: &mut HashMap<ShapeType, Handle<Mesh>>,
        instanced_materials: &mut InstancedMaterials,
        shape: &dyn Shape,
...
#[cfg(feature = "dim2")]
fn bevy_mesh(buffers: (Vec<Point2<Real>>, Vec<[u32; 3]>)) -> Mesh {
    bevy_mesh2d(buffers)
}
...
fn bevy_mesh2d(buffers: (Vec<Point2<Real>>, Vec<[u32; 3]>)) -> Mesh {
    bevy_mesh3d((
        buffers
            .0
            .into_iter()
            .map(|pt| pt.coords.push(0.0).into())
            .collect(),
        buffers.1,
    ))
}

fn bevy_mesh3d(buffers: (Vec<Point3<Real>>, Vec<[u32; 3]>)) -> Mesh {
    let (vtx, idx) = buffers;
    let mut normals: Vec<[f32; 3]> = vec![];
    let mut vertices: Vec<[f32; 3]> = vec![];
...
pub fn update_rigid_graphics(
    physics_context: ResMut<PhysicsContext>,
    rigid_render: Res<RenderContext>,
    mut transforms: Query<&mut Transform>,
) {
    for object in &rigid_render.rigid_entities {
        object.update(
            &physics_context.rapier_data.colliders,
            &mut transforms,
            &Default::default(),
        );
    }
